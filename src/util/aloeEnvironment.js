...
const getAloeVersion = () => {
  let version = null;
  const exePath = getExecutablePath('aloe');
  // first see if we can get a aloe exe in a standard location relative to where we are
  try {
    ...
    // that didn't work, let's try as if we're in the venv or aloe is on the path
    try {
      version = child_process
