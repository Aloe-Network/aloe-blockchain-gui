...
import aloeEnvironment from '../util/aloeEnvironment';
import aloeConfig from '../util/config';
...
if (!handleSquirrelEvent()) {
  ...
  const ensureCorrectEnvironment = () => {
    ...
    if (!aloeEnvironment.guessPackaged() && !('VIRTUAL_ENV' in process.env)) {
  ...
  if (ensureSingleInstance() && ensureCorrectEnvironment()) {
    ...
    const createWindow = () => {
      ...
      if (!aloeConfig.manageDaemonLifetime()) {
      ...
      mainWindow.on('close', (e) => {
        ...
        if (decidedToClose || !aloeConfig.manageDaemonLifetime()) {
          return;
        }
        e.preventDefault();
        if (!isClosing) {
    ...
    const appReady = async () => {
      ...
      // if the daemon isn't local we aren't going to try to start/stop it
      if (aloeConfig.manageDaemonLifetime()) {
        aloeEnvironment.startAloeDaemon();
      }
    };
  ...
  const getMenuTemplate = () => {
    const template = [
      ...
      },
      {
        ...
        submenu: [
          {
            label: i18n._(/* i18n */ { id: 'Aloe Blockchain Wiki' }),
            click: () => {
              openExternal(
                'https://github.com/Aloe-Network/aloe-blockchain/wiki',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Aloe-Network/aloe-blockchain/wiki/FAQ',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Aloe-Network/aloe-blockchain/releases',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Aloe-Network/aloe-blockchain/blob/master/CONTRIBUTING.md',
              );
            },
          },
          {
          ...
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Aloe-Network/aloe-blockchain/issues',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal('https://keybase.io/team/aloe_network.public');
            },
          },
          {
            label: i18n._(/* i18n */ { id: 'Follow on Twitter' }),
            click: () => {
              openExternal('https://twitter.com/aloe_project');
            },
          },
        ],
      },
    ...
    if (process.platform === 'darwin') {
      // Aloe Blockchain menu (Mac)
      template.unshift({
        label: i18n._(/* i18n */ { id: 'Aloe' }),
        submenu: [
          {
            label: i18n._(/* i18n */ { id: 'About Aloe Blockchain' }),
            click: () => {
              openAbout();
            },
          },
